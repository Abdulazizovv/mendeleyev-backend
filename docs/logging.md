...
- Gunicorn access/error loglari STDOUT/STDERR ga chiqariladi (`docker logs` orqali ko'rinadi)
- Konteyner ishga tushganda `/usr/src/app/logs` papkasi yaratiladi

## Konfiguratsiya (`settings.py`)

- `LOGGING` dict — formatters, handlers (console, TimedRotatingFileHandler), loggers (`django`, `django.request`, `celery`, `apps`)
