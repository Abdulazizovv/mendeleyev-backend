...
class AdminProfileTests(TestCase):
    ...
    def test_auto_create_admin_profile_for_branch_admin(self):
        m = UserBranch.objects.create(user=self.user_admin, branch=self.branch, role=BranchRole.BRANCH_ADMIN)
    ...
    def test_auto_create_admin_profile_for_super_admin(self):
        m = UserBranch.objects.create(user=self.user_super, branch=self.branch, role=BranchRole.SUPER_ADMIN)
        self.assertTrue(hasattr(m, 'admin_profile'))
    ...
    def test_role_change_teacher_to_branch_admin_creates_admin_profile(self):
        ...
        m = UserBranch.objects.create(user=self.user_teacher, branch=self.branch, role=BranchRole.TEACHER)
        ...
        # Refresh from db
        m_ref = UserBranch.objects.get(id=m.id)
