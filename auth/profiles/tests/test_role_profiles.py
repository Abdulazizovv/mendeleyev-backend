...
class RoleProfilesTests(TestCase):
    ...
    def test_auto_create_teacher_profile(self):
        m = UserBranch.objects.create(user=self.teacher, branch=self.branch, role=BranchRole.TEACHER)
    ...
    def test_auto_create_student_profile(self):
        m = UserBranch.objects.create(user=self.student, branch=self.branch, role=BranchRole.STUDENT)
        self.assertTrue(hasattr(m, 'generic_profile'))
        self.assertTrue(hasattr(m, 'student_profile'))

    def test_auto_create_parent_profile(self):
        m = UserBranch.objects.create(user=self.parent, branch=self.branch, role=BranchRole.PARENT)
        self.assertTrue(hasattr(m, 'generic_profile'))
